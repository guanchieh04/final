...
choices = ['國泰: 2023.4.17 至 2025.4.16','金融期貨FXF:2020.3.2至2025.4.14','台積電期貨CDF:2022.3.2至2025.4.14','小型臺指MXF:2020.3.2至2025.3.14' ]
...
if choice == '國泰: 2023.4.17 至 2025.4.16':
    df_original = pd.read_csv('future_KBar_CCF.pkl')
    product_name = '國泰CCF'
...
#     df_original = load_data('kbars_TXF202412_2024-01-01-2024-04-09.pkl')  
    df_original['time'] = pd.to_datetime(df_original['time'])  # 就能直接轉換
    df_original.columns
    df_original.head()

if choice == '金融期貨FXF:2020.3.2至2025.4.14':
    df_original =pd.read_csv('future_KBar_FXF.pkl')
    product_name = '金融期貨FXF'
    df_original['time'] = pd.to_datetime(df_original['time'])  # 就能直接轉換
    df_original.columns
    df_original.head()
if choice == '台積電期貨CDF:2022.3.2至2025.4.14':
    df_original =pd.read_csv('future_KBar_CDF.pkl')
